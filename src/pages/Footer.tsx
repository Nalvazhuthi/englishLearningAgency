...
const Footer = () => {
  const sections = [
    ...
    {
      header: "Product",
      sections: ["Features", "Work", "Career", "Money"],
    },
    {
      header: "Get in Touch",
  ...
  return (
    <div className="footer-container">
      <div className="heading">
        Open the door to a <div className="highlight"> new world</div> by
        learning English
      </div>
      <button>Get Started</button>
      <div className="section-wrapper">
        {sections.map((section, index) => (
          <div className="each-section" key={`${section.header}`}>
            <div className="section-header">{section.header}</div>
            <div className="section-navs">
              {section.sections.map((sec) => (
                <div className="section-nav" key={`${sec}`}>
                  {sec}
                </div>
              ))}
            </div>
